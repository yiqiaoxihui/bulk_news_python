...
def ted(begin_page,end_page,type_name):
	headers = {}
	...
	file_name='ted_'+type_name+'_page_'+str(begin_page)+"_to_"+str(end_page)+"_"+str(int(time.time()))+".txt"
	count=0
	dic={}
	dic['status']=1
	...
	for i in range(begin_page,end_page+1):
		try:
			url = 'https://www.ted.com/talks?duration=6-12&language=en&page='+str(i)+'&sort=newest'
			...
			# print soup
			try:
				for item in soup.find_all(name='div',attrs={"class":"media__message"}):
					# print("*************************")
					# print item.string.encode('utf-8')
					a=item.find('a',attrs={"class":"ga-link"})
					time_lable=item.find('span',attrs={"class":"meta__val"})
