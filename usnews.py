...
def download_usnews_national_news(offset,count,typ):
	headers = {}
	...
	file_name='usnews_'+typ+'_'+otherStyleTime+".txt"
	fw=open("download/"+file_name,'w')
	
	while current<count:
		# print current
		try:
			url = 'https://www.usnews.com/news/'+typ+'?offset='+str(offset+current)+'&renderer=json'
			# print url
...
download_usnews_national_news(sys.argv[1],sys.argv[2],sys.argv[3])