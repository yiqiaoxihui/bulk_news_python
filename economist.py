...
def economist(begin_page,end_page,type_name):
	headers = {}
	...
	file_name='economist_'+type_name+'_page_'+str(begin_page)+"_to_"+str(end_page)+".txt"
	count=0
	dic={}
	dic['status']=1
	...
	for i in range(begin_page,end_page+1):
		time.sleep(2)
		try:
			url = 'https://www.economist.com/'+type_name+'/?page='+str(i)
			...
			# print soup
			try:
				for item in soup.find_all(name='article',attrs={"class":"teaser"}):
					# print("*************************")
					# print item.string.encode('utf-8')
					title=''
					title_span=item.find('span',attrs={'class':'flytitle-and-title__title'})
					if title_span:
						title=title_span.text.encode('utf-8')
					else:
						print 'no find title span'
					time_lable=item.find('time',attrs={"itemprop":"dateCreated"})
					if time_lable:
						date=time_lable['datetime'].encode('utf-8').strip().split('T')[0]
					else:
						print 'no find time'
...
economist(sys.argv[1],sys.argv[2],sys.argv[3])
