...
def wsj_opinion(begin_page,end_page,type_name):
	...
	file_name='wsj_opinion_'+type_name+".txt"
	count=0
	dic={}
	dic['status']=1
	...
	for i in range(begin_page,end_page+1):
		try:
			url = 'https://www.wsj.com/search/term.html?isAdvanced=true&articleType='+type_name_dic[type_name]+'&daysback=4y&min-date=2015/04/19&max-date=2019/04/19&source=wsjarticle,wsjblogs,wsjvideo,sitesearch&page='+str(i)
			print url
			req = requests.get(url, headers=headers, timeout=60)
			# new_doc=UnicodeDammit.detwingle(req.text.encode('utf-8'))
			# fw.seek(155417)
			# print(fw.readline())
			# fw.write(req.text.encode('utf-8'))
			# fw.close()
			# data = open("q_format.html").read()
			# data='レイヤー_1'
			# fencoding=chardet.detect(data)
			# data=unicode(data,errors='replace')
			# data=data.decode(fencoding['encoding']).encode("utf-8")
			# print(fencoding['encoding'])
			# data=data.decode('unicode-escape')
			# print data
			# diagnose(req.text)
			# diagnose(req.text.encode('utf-8'))
...
s='commentary-u-s'
wsj_opinion(sys.argv[1],sys.argv[2],sys.argv[3])
